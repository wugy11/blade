...
public class RequestTest extends BaseTestCase {
	...
	public void testPathParam() throws Exception {
		start(app.get("/user1/:id", (req, res) -> res.text(req.pathInt("id").toString()))
				.get("/user2/:id", (req, res) -> res.text(req.pathLong("id").toString())).get("/user3/:name/:age",
						(req, res) -> res.text(req.pathString("name") + ":" + req.pathString("age"))));
	...
	public void testPathParams() throws Exception {
		start(app.get("/user1/:id", (req, res) -> res.json(req.pathParams())).get("/user2/:name/:age",
				(req, res) -> res.json(req.pathParams())));
	...
	public void testHost() throws Exception {
		start(app.get("/", (request, response) -> System.out.println(request.host())));
		bodyToString("/");
	}

	@Test
	public void testUri() throws Exception {
		start(app.get("/a", (request, response) -> response.text(request.uri()))
	...
	public void testUrl() throws Exception {
		start(app.get("/hello", (request, response) -> response.text(request.url())));
		assertThat(bodyToString("/hello?name=q1"), is("/hello?name=q1"));
	}

	@Test
	public void testUserAgent() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.userAgent())));
	...
	public void testProtocol() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.protocol())));
		assertThat(bodyToString("/"), is("HTTP/1.1"));
	}

	@Test
	public void testQueryString() throws Exception {
		start(app.get("/hello", (request, response) -> response.text(request.queryString())));
		assertThat(bodyToString("/hello?name=q1"), is("/hello?name=q1"));
	}

	@Test
	public void testQueryParam() throws Exception {
		start(app.get("/query", (request, response) -> response.text(request.query("name", "jack")))
	...
	public void testHttpMethod() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.method()))
	...
	public void testAddress() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.address())));
	...
	public void testContentType() throws Exception {
		start(app.get("/c1", (request, response) -> response.text(response.contentType())).get("/c2", (request,
				response) -> response.contentType("application/json; charset=UTF-8").text(response.contentType())));
	...
	public void testIsSecure() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.isSecure() + "")));
	...
	public void testIsAjax() throws Exception {
		start(app.get("/a1", (request, response) -> response.text(request.isAjax() + "")).get("/a2",
				(request, response) -> response.text(request.isAjax() + "")));
	...
	public void testIsIE() throws Exception {
		start(app.get("/", (request, response) -> response.text(request.isIE() + "")));

		assertThat(get("/").userAgent(firefoxUA).body(), is("false"));
		assertThat(get("/").userAgent("Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0)").body(),
				is("true"));
	}

	@Test
	public void testHeaders() throws Exception {
		start(app.get("/", (request, response) -> response.json(request.headers())));

		String body = get("/").header("h1", "a1").header("h2", "a2").header("h3", "a3").body();
	...
	public void testHeader() throws Exception {
		start(app.get("/", (request, response) -> response.json(request.header("h1"))));
	...
	public void testKeepAlive() throws Exception {
		start(app.get("/", (request, response) -> response.json(request.keepAlive() + "")));
	...
	public void testAttribute() throws Exception {
		start(app.get("/", (request, response) -> request.attribute("name", "jack")).after("*",
				((request, response) -> {
	...
	public void testFileItems() throws Exception {
		start(app.post("/upload1", (request, response) -> {
		...
		assertThat(body, is(
				"{\"file1\":{\"fileName\":\"a.txt\",\"name\":\"file1\",\"length\":1551,\"contentType\":\"text/plain\"}}"));

		body = post("/upload2").part("file1", "a.txt", new File(Const.CLASSPATH + "log_config.txt")).body();
		assertThat(body,
				is("{\"fileName\":\"a.txt\",\"name\":\"file1\",\"length\":1551,\"contentType\":\"text/plain\"}"));
	}
