...
public class AsonArrayTest {
	...
	@Before
	public void setup() {
		array = new AsonArray<Ason>().add(new Ason().put("_id", 1).put("name", "Aidan").put("attrs.priority", 2))
	...
	public void get_object_test() {
		...
		assertNotNull(ason);
		assertEquals(1, ason.getInt("_id"));
	}
	...
	public void test_array_in_array_deserialize() {
		AsonArray<Integer[]> parent = new AsonArray<Integer[]>().add(new Integer[] { 1, 2, 3, 4 },
				new Integer[] { 5, 6, 7, 8 });
		assertEquals(2, parent.size());
