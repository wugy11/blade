...
public class AsonPathTest {
	...
	public void builder_test() {
		...
		assertEquals(3, ason.getInt("person._id"));
		assertEquals("Aidan", ason.get("person.name"));
		assertEquals(21, ason.getInt("person.age"));
	...
	public void builder_index_test_one() {
		Ason ason = new Ason().put("_id", 3).put("name", "Aidan").put("pets.$0", "Kierra").put("pets.$1", "Elijah")
				.put("pets.$2", "Olivia");
		assertEquals("{\"pets\":" + "[\"Kierra\",\"Elijah\",\"Olivia\"]," + "\"name\":\"Aidan\",\"_id\":3}",
				ason.toString());
	}

	@Test
	public void builder_index_test_two() {
		Ason ason = new Ason().put("_id", 3).put("name", "Aidan").put("pets.$0.id", 1).put("pets.$0.name", "Kierra")
				.put("pets.$1.id", 2).put("pets.$1.name", "Elijah").put("pets.$2.id", 3).put("pets.$2.name", "Olivia");
		assertEquals("{\"pets\":[" + "{\"name\":\"Kierra\",\"id\":1}," + "{\"name\":\"Elijah\",\"id\":2},"
				+ "{\"name\":\"Olivia\",\"id\":3}]," + "\"name\":\"Aidan\",\"_id\":3}", ason.toString());
	}

	@Test
	public void builder_index_test_three() {
		Ason ason = new Ason().put("_id", 1).put("people.$0.name", "Aidan").put("people.$0.pets.$0", "Kierra")
				.put("people.$0.pets.$1", "Elijah").put("people.$0.pets.$2", "Olivia");
		assertEquals("{\"_id\":1," + "\"people\":[" + "{\"pets\":" + "[\"Kierra\",\"Elijah\",\"Olivia\"],"
				+ "\"name\":\"Aidan\"}" + "]" + "}", ason.toString());
	}

	@Test
	public void builder_index_test_four() {
		Ason ason = new Ason().put("_id", 1).put("people.$0.name", "Aidan").put("people.$0.id", 1)
				.put("people.$0.pets.$0.name", "Kierra").put("people.$0.pets.$0.id", 1)
				.put("people.$0.pets.$1.name", "Elijah").put("people.$0.pets.$1.id", 2);
		assertEquals("{\"_id\":1," + "\"people\":[" + "{\"pets\":" + "[{\"name\":\"Kierra\",\"id\":1},"
				+ "{\"name\":\"Elijah\",\"id\":2}]," + "\"name\":\"Aidan\"," + "\"id\":1}]}", ason.toString());
	}
	...
	public void test_index_notation() {
		String input = "{\"group_id\":1,\"title\":\"Hello, world!\","
				+ "\"participants\":[{\"name\":\"Aidan\",\"id\":2}," + "{\"name\":\"Waverly\",\"id\":1}]}";
		Ason object = new Ason(input);

		assertEquals("Waverly", object.get("participants.$1.name"));
		assertEquals(2, object.getInt("participants.$0.id"));
	}
	...
	public void test_remove_dot_notation() {
		Ason ason = new Ason().put("_id", 3).put("name", "Aidan").put("age", 21).put("spouse.name", "Waverly")
				.put("spouse.age", 19);
		ason.remove("spouse.age");
		ason.remove("spouse.nonexisting.test"); // nothing should happen here
		assertEquals("{\"name\":\"Aidan\",\"_id\":3,\"age\":21," + "\"spouse\":{\"name\":\"Waverly\"}}",
				ason.toString());
	}
	...
	public void test_remove_index_notation() {
		...
		assertEquals(participants.get(0).getInt("id"), 1);
	...
	public void test_mid_path_null() {
		Ason ason = new Ason().put("person.name", "Aidan").put("person.born", 1995).put("person.spouse.name",
				"Waverly");
		assertEquals("Aidan", ason.get("person.name"));
	...
	public void test_index_notation_mid_null() {
		Ason ason = new Ason().putNull("person.family").putNull("person.props.$0").putNull("person.props.$1");
	...
	public void test_error_get_array_keyname() {
		...
		try {
			ason.get("array.$1i");
			assertFalse("No error was thrown for attempting" + " to retrieve a value from an array using a name key!",
					false);
