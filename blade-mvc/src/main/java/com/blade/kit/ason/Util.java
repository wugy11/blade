...
public class Util {
	...
	public static Object followPath(JSONObject wrapper, String key, String[] splitKey, boolean createMissing) {
		...
		if (!isNull(parent) && !(parent instanceof JSONObject) && !(parent instanceof JSONArray)) {
			throw new InvalidPathException("First component of key " + key + " refers to " + splitKey[0]
					+ ", which is not an object or array (it's a " + parent.getClass().getName() + ").");
	...
	public static <T> T getPathValue(JSONObject wrapper, String key, String[] splitKey) {
		...
		} else if (target instanceof JSONArray && lastKey.startsWith("$") && StringKit.isNumber(lastKey.substring(1))) {
	...
	public static boolean shouldIgnore(Field field) {
		return field.getName().startsWith("this$") || field.getName().equals("$jacocoData")
	...
	public static void setFieldValue(Field field, Object object, Object value) {
		...
		try {
			...
			throw new RuntimeException(
					"Failed to set the value of " + field.getName() + " in " + object.getClass().getName(), e);
		}
	}
