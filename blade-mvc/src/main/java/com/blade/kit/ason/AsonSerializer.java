...
public class AsonSerializer {
	...
	public Ason serialize(Object object) {
		if (Util.isNull(object)) {
			return null;
		} else if (object instanceof Ason || object instanceof AsonArray || object instanceof JSONObject
				|| object instanceof JSONArray) {
			throw new IllegalArgumentException("You cannot serialize Ason or AsonArray.");
		} else if (ClassKit.isPrimitive(object)) {
	...
	public AsonArray<Object> serializeArray(Object arrayObject) {
		...
		for (int i = 0; i < length; i++) {
			...
			Class<?> itemCls = cls.getComponentType() == Object.class && value != null ? value.getClass()
					: cls.getComponentType();
			if (ClassKit.isPrimitive(itemCls)) {
				result.add(value);
				continue;
			}
	...
	public Object serializeField(final Field field, final Object object) {
		...
		if (ClassKit.isPrimitive(fieldValue) || fieldValue instanceof JSONObject || fieldValue instanceof JSONArray
				|| fieldValue instanceof Ason || fieldValue instanceof AsonArray) {
			return fieldValue;
	...
	public <T> T deserialize(Ason ason, Class<T> cls) {
		if (isNull(ason)) {
			return null;
		} else if (ClassKit.isPrimitive(cls)) {
			throw new IllegalArgumentException(
		...
		for (String name : cacheEntry.fields()) {
			...
			if (ClassKit.isPrimitive(type) || type == JSONObject.class || type == JSONArray.class || type == Ason.class
	...
	public <T> T deserializeArray(AsonArray<?> json, Class<T> cls) {
		...
		for (int i = 0; i < json.size(); i++) {
			...
			if (isNull(item)) {
				Array.set(newArray, i, ClassKit.defaultPrimitiveValue(component));
				continue;
			}

			final Class<?> itemType = component == Object.class ? item.getClass() : component;
			if (ClassKit.isPrimitive(itemType)) {
				...
				throw new IllegalStateException("Expected JSON array to contain JSON objects " + "to deserialize to "
						+ itemType.getName() + ", found " + item.getClass().getName() + " objects instead.");
			} else {
